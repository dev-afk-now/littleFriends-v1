...
struct MapView: View {
    ...
    var body: some View {
        ...
        ZStack {
            Map(coordinateRegion: $centerCoordinate, interactionModes: MapInteractionModes.all, showsUserLocation: true, userTrackingMode: $trackingMode, annotationItems:
                    filterAnimal.isEmpty ? showAll ? viewModel.places : viewModel.places.filter { $0.distance(to: CLLocation(latitude: userLocation?.latitude ?? 0, longitude: userLocation?.longitude ?? 0)) < searchRange
            } : showAll ? viewModel.places.filter{ $0.animalType == filterAnimal } : viewModel.places.filter{ $0.distance(to: CLLocation(latitude: userLocation?.latitude ?? 0, longitude: userLocation?.longitude ?? 0)) < searchRange
