    ...
    var body: some View {
            ...
            VStack {
                VStack {
                    ...
                    HStack {
                        Text("\(item.isFounded ? "Founded at " : "Posted at ")\(item.formattingDate(date: item.publishTime, withoutTime: true)!)").font(.custom("AppleSDGothicNeo-thin", size: 22, relativeTo: .title)).padding(.horizontal, 8)
                        Spacer()
                    }
                }.padding(.horizontal)
